...
seq_test = pad_sequences(test_seq, maxlen=102, padding='pre',)
...
img_test = imgs_test.reshape(602,128,128,3).astype('float')
print(type(y))
...
merge3 = Bidirectional(LSTM(128))(merge2)
print('LSTM의 모양',merge2.shape)
#shape=(None, 256)
# merge3 = Reshape((-1,))(merge2)
merge4= Dense(256, activation='relu')(merge3)
last_output= Dense(128, activation='relu')(merge4)
...
y_predict = model.predict([seq_test,img_test])
submission['ST1_GAP(eV)'] = y_predict
submission.to_csv('../data/dacon3/dacon_baseline(tensorversion).csv', index=False)
