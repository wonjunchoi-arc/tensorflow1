...
# factor =0.5  5번 변화가 없으면 50%만큼 줄인다는 뜻이다!!
#즉 0.1 -> 0.05 -> 0.025
...
#컴파일에서 optimizer를 설정했지만 실제로 적용되는 곳은 fit이다. 즉 Callback에서 적용


hist = model.fit(x_train,y_train, epochs=300,
batch_size=512, validation_split=0.25,verbose=1,callbacks=[es, reduce_lr])
